...
export interface VerifyRouteContext {
  ...
  allowedClients: string[];           // ✅ add this
}
...
// ✅ Require client to be registered in facilitator allowlist
if (!context.allowedClients.includes(clientPublicKey)) {
  return res.json({
    isValid: false,
    error: 'Client public key is not registered or authorized',
  });
}
